...
```bash
find <каталог> -type f -exec du -h {} + | sort -h | tail -n 1 > biglog.txt
```
...
```bash
find <каталог> -type f -name "*.log" | wc -l > log_count.txt
```
...
```bash
grep "host" <каталог>/*.conf > host_params.txt
```
...
```bash
zip -r config_backup.zip <каталог>
```
...
zip project_backup.zip <каталог>/config/*.conf <каталог>/**/*.log errors.txt
```
...
```bash
grep -v '^$' <каталог>/app.logs > cleaned_app.log
```
...
```bash
wc -l <каталог>/*.conf > conf_stats.txt
```
